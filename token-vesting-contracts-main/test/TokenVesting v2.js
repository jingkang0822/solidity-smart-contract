...
describe("TokenVesting", function () {
  ...
  describe("Vesting", function () {
    ...
    it("Should vest tokens gradually", async function () {
       ...
       * 
       * Questions:
       * 1. How if the time pass is no integer portion of the duration,
       *    the calculation of computeReleasableAmount can be correct?
       * 
       * 2. How if one beneficiary does have more vestSchedule than 1,
       *    then calculation of computeReleasableAmount will loop all schedule?
       * Ans: Need manually loop the schedules, since release function depend on vestingScheduleId
       * 
       * 3. What if createVestingSchedule revoke set to false, what is the revoke means?
       * Ans: Revoke means early terminate one beneficiary schedule, 
       *      only release the time before, after the time can not release anymore.
       */
    });
    ...
    it("Should compute vesting schedule index", async function () {
      ...
      // Compute encodePacked with js
      console.log(`Address: ${addr1.address}`);
      console.log(`abi.encodePacked: ${ethers.utils.solidityPack(["address", "uint256"], [addr1.address, 0])}`);
      console.log(ethers.utils.keccak256(ethers.utils.solidityPack(["address", "uint256"], [addr1.address, 0])));
